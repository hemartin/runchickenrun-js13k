...
function Body(state) {
    ...
    // target, where body is heading to (if targetRadius > 0)
...
Body.prototype.applyForces = function (timestep) {
    if (this.targetRadius > 0) {
        // calculate force
        var force_x = this.target.x - this.origin.x;
        var force_y = this.target.y - this.origin.y;
        var force_len = Vector_length(force_x, force_y);
        force_x *= this.thrust / force_len;
        force_y *= this.thrust / force_len;

        // apply force
        this.velocity.x += force_x * timestep * this.invertedMass();
        this.velocity.y += force_y * timestep * this.invertedMass();
    }

    // apply friction
    this.velocity.scale(Math.max(0, 1 - timestep * LATERAL_FRICTION));
    this.angularVelocity *= Math.max(0, 1 - timestep * ROTATIONAL_FRICTION);
};
...
Body.prototype.checkTarget = function () {
    if (this.targetRadius > 0) {
        var dist = this.target.distanceFrom(this.origin);
